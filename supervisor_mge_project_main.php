...
<p>Here you can manage (view) Project(s).</p>
...
<form name="frmSearch" onSubmit="JavaScript: location.href = 'supervisor_mge_project_main.php?name=' + frmSearch.txtName.value; return false;">
...
<form id="frm"  name="frm" method="post" action="supervisor_mge_project_del_sel.php" onSubmit="return DeleteAll()" >
...
if(isset($_GET['name']) && $_GET['name'] != '')
{
	...
	$sql = 'SELECT project_id, pj.name pj_name, sp.name sp_name, st.name st_name ' . 
		...
		'WHERE pj.supervisor_id = ' . $_SESSION['supervisor_id'] . 
...
else
{
	...
	$sql = 'SELECT project_id, pj.name pj_name, sp.name sp_name, st.name st_name ' . 
		...
		'LEFT JOIN supervisor sp ON pj.supervisor_id = sp.supervisor_id) ' . 
		'LEFT JOIN student st    ON pj.student_id    = st.student_id ' . 
		'WHERE pj.supervisor_id = ' . $_SESSION['supervisor_id'] . ' ORDER BY project_id DESC;'; 
...
else
{
	...
	while($row = mysql_fetch_array($result))
	{
		...
		<tr id="row<?php echo $count; ?>" onmouseover="over(this.id,'lightgreen')" onmouseout="out(this.id,'#CCCCFF')">
			...
			<td align="center">
				<input type="image" onClick="JavaScript: location.href = 'supervisor_mge_project_show.php?project_id=' + this.value; return false;" src="images/b_browse.png" title="Browse record" value="<?php echo $row['project_id'] ?>" />
				&nbsp;
			</td>
		</tr>
		<?php
